...
lead: アプリまたはこのページのフォームから、アカウントと関連データを削除できます。
...
- 閲覧履歴、マイリスト（価格監視・目標価格を含む）
- 節約記録、買い時ノート（相場の自動チェック対象を含む）
- 家族に共有した自分の記録。参加中の家族からも抜けます（最後の1人の場合は家族ごと削除）
- 通知設定、スキャン回数・読み取りポイント

Premiumを購入している場合、アカウントを削除しても課金は停止しません。先にApp StoreまたはGoogle Playで解約してください。
